...
const HOVER_STYLEBOX := preload("res://scenes/card_ui/card_hover_style_box_flat.tres")
...
var original_index := self.get_index()
var parent: Control
var tween: Tween
...
func _on_card_drag_or_aiming_started(used_card: CardUI) -> void:
	# disables all other cards while dragging the current one
	# prevents the player from interacting with multiple cards
	# at the same time.
	if used_card == self:
		return
